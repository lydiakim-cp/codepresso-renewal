...
description: 순수 HTML/CSS/바닐라 JS(Tailwind·React 없이 직접 CSS를 짜는) 프로젝트의 마크업·스타일 규범과 디자인 QA 기준. 제1규칙은 "새로 만들지 말고 기존 컴포넌트·토큰·패턴을 찾아 그대로 재사용". HTML/CSS를 쓰거나 고칠 때, 섹션·서브페이지를 만들거나 리디자인할 때, 화면이 심심해 배경·아이콘·모션을 보강할 때, 목업에 연출을 넣을 때, 반응형·CSS 중복 정리·리팩터링·커밋을 할 때, 그리고 **작업을 마친 뒤 색·구조·배치를 디자이너 시점으로 QA·검수할 때**(강조 섹션·강조 배경 폭·움직이는 목업·애니메이션 점검) 항상 먼저 로드한다. 이 스킬의 구체 토큰값·컴포넌트명·어휘표는 "코드프레소" 프로젝트 기준이며, 다른 프로젝트에서는 [references/porting.md](references/porting.md)를 따라 자사 값으로 교체해 쓴다.
---
...
### 0-1. "A를 B로 바꿔줘" = A의 디자인을 버리고 B로 갈아탄다

사용자가 **"a를 b가 쓰는 class로 바꿔줘"**라고 하면 클래스 이름만 바꾸는 것이 아니다.
**마크업 구조·CSS·디자인 전부를 B의 것으로 교체한다.**

- 기존 A의 디자인이 B와 다른 것은 **당연한 전제**다. 그 다름을 지키려고 페이지 스코프에
  A의 값(크기·색·그림자·배경)을 남겨 두면 **잘못한 것이다.** 지시는 "A를 버려라"였다.
- 마크업도 B의 Markup API를 그대로 따른다. B가 결론 칸을 `is-return`으로 표현하면
  A의 `:last-child` 페이지 규칙을 지우고 `is-return`을 붙인다.
- 페이지 스코프에 남길 수 있는 것은 **B의 CSS 주석이 "사용 페이지가 정한다"고 명시한
  것**(열 수·delay 등)과 **사용자가 이번에 따로 지시한 예외**뿐이다.
- 반응형(`css/mobile.css`)의 A용 수치도 B의 실제 크기에 맞춰 다시 계산한다.

같은 지시가 "이 컴포넌트 써", "b처럼 해줘"로 와도 같다.
...
> 🔧 아래 구체 수치(14px·900/720/560 등)만 자사 값으로 바꾼다(→ [references/porting.md](references/porting.md)). 규칙 형식 자체는 그대로 쓴다.
...
**요청하지 않은 값을 얹지 않는다**
- **지시에 없는 속성을 "좋아 보여서" 추가하지 않는다.** 특히 `max-width`(`72ch` 등
  읽기 폭 제한) · `min-height` · 여분의 `padding` · `letter-spacing`. 사용자는 매번
  이런 것을 손으로 걷어내 왔다.
- **폭은 컨테이너(`--container-*`)와 그리드가 정한다.** 요소에 `max-width`를 새로
  붙이면 부모가 잡아 둔 정렬이 그 요소에서만 어긋난다.
- 정말 필요하다고 판단되면 **넣지 말고 보고에 한 줄로 제안한다** — "여기 폭 제한이
  있으면 좋겠는데 넣을까요". 넣어 놓고 나중에 지우는 것이 가장 비싸다.
...
- **구분선은 예외다.** 영역을 나누는 선은 계속 쓴다 — 섹션 경계, 표의 행 사이,
  레일과 본문 사이(`tab-catalog__tabs`), 목업 상단 바(`preview-frame__bar`),
...
**주석**
- **CSS·HTML 안 주석은 한 줄(라인)에 100자 이내로 쓴다.** 여러 줄로 나눠 쓰지 말고
  핵심(무엇을·왜)만 한 줄로 압축한다 — 장황한 배경 설명·과거 시행착오 나열은
  git 커밋 메시지에 남기고, 코드 옆 주석은 다음에 읽을 사람이 1초 안에 판단을
  내릴 수 있는 만큼만 남긴다.
...
- **표에 없는 역할이 나오면** 먼저 가장 가까운 이름에 한 단어를 붙여 본다
  (`catalog-pricing` · `features-team`). 그래도 역할이 달라 이름이 거짓이 되면
  **임의로 만들지 말고 사용자에게 묻고** 확인 후 표에 등록한다.
...
HTML·CSS를 저장하면 `.claude/settings.json`의 훅이
`scripts/gate.js`를 돌린다(`token-lint` + `page-audit`). 위반이 있으면 그 자리에서
되돌아오므로, 아래 규칙 중 **기계가 잡는 것은 문서를 다시 읽지 않아도 지켜진다.**

| 검사 | 스크립트 | 잡는 것 |
|---|---|---|
| 임의의 값 | `token-lint.js` | 정의 안 된 `var()`, raw 색, `font-size`·`font-weight`·`border-radius` 리터럴, --space 밖 여백 |
| 임의의 디자인 | `page-audit.js` | **CSS에 없는 클래스**(오타·새 디자인), 링크 3개와 순서, 페이지 스코프, 다크 짝, `data-*` ↔ 스크립트, meta, 없는 파일 참조, `mobile.css` 구조 |
| 대비 | `contrast-audit.js` | 면 위 텍스트 WCAG 대비(수동 실행) |

- **기존 위반은 `token-lint.baseline.json`에 기록**돼 있어 새로 생긴 것만 막는다.
  백로그를 볼 때는 `--all`, 백로그를 줄인 뒤에는 `--write-baseline`으로 갱신한다.
- 게이트가 잡지 못하는 것 — **어떤 컴포넌트를 고르는지, 강조를 어디 한 번 주는지,
  화면이 지루한지.** 그건 여전히 아래 규칙과 사람의 눈이 판단한다.
