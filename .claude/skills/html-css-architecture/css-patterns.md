...
> 🔧 표시 블록만 다른 프로젝트에서 자기 값으로 바꾼다(→ [references/porting.md](references/porting.md)). 나머지는 그대로 쓴다.
...
> 🔧 아래 px 값·아이콘 파일명·색 배정표는 자사 아이콘셋으로 바꾼다. 크기를 소수
> 클래스로 고정하는 원칙 자체는 그대로 쓴다.
...
> 🔧 아래 실측 대비비는 코드프레소 hex 기준 — 자사 팔레트로 다시 잰다(절차는
> [surface-depth.md](references/surface-depth.md) 참고).
>
