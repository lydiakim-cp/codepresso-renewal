...
> 읽는 법 — 이 문서는 범용 방법론과 코드프레소 전용 값이 섞여 있다.
> "🔧 코드프레소 값 — 교체 지점" 표시가 붙은 블록만 다른 프로젝트에서 자기 값으로
> 바꾼다. 표시 없는 본문(색을 파생시키는 방식·아이콘 체계화 원칙·모션 이중 리듬 등)은
> 그대로 가져가 쓴다. → [references/porting-guide.md](references/porting-guide.md)
...
> 🔧 **코드프레소 값 — 교체 지점.** 크기를 소수의 고정 클래스로 제한하고 박스와
> 글리프를 짝으로 관리한다는 원칙, 방향 아이콘을 SVG 하나로 통일한다는 원칙, 외부
> 아이콘의 하드코딩 색을 브랜드 토큰으로 교체한다는 원칙은 범용이다. 아래
> px 값(20/24/32)·아이콘 파일명(`ic_barChart` 등)·색 배정표는 자사 아이콘셋으로
> 바꾼다.

### 아이콘 크기 — 세 클래스로만 정한다

**크기는 CSS가 아니라 마크업의 클래스가 말한다.** 컴포넌트 CSS에서 아이콘에
`width`/`height`를 다시 쓰지 않는다(`components/ui/icon.css`).

| 클래스 | 크기 | 쓰는 자리 |
|---|---|---|
| `.icon-sm` | 20px | 라벨·텍스트 옆 인라인 — GNB 메가메뉴 항목, FAQ 화살표, 버튼 안 |
| `.icon-md` | 24px | 기본 — 카드·스텝의 아이콘 박스 안, 제목 옆 |
| `.icon-lg` | 32px | 한 칸의 주인공인 아이콘 — hero 보조 밴드 |

- **이 셋 말고 다른 크기를 만들지 않는다.** 26·30·45px로 흩어져 있던 것을
  한 번 정리했다.
- **감싼 `<span>`에 붙이면** 안쪽 `svg`·`img`가 같은 크기로 따라온다.
...
<span class="hero-question-icon icon-lg" aria-hidden="true"><svg …></span>
```

- **반응형에서 아이콘을 줄이지 않는다.** 720px에서 한 단 낮추던 오버라이드를
  지웠다 — 단이 늘어나는 통로였다.
- **목업 안은 이 스케일 밖이다.** `product-mock`·`difference-mock`의 축소 아이콘은
  실제 제품 화면을 축소해 그린 것이라 별도 스케일을 갖는다.
...
> 🔧 **코드프레소 값 — 교체 지점.** "옅은 면과 흰 면을 번갈아 배치해 위계를
> 만든다"는 원칙과, 그 절차(자기 팔레트의 흰색 대비비를 실측해 뭉친 톤을 찾는
> 방법)는 범용이다. 실측 대비비 표는 코드프레소 hex 기준이므로 자사 팔레트로
> 다시 재야 한다 — 이 절차 자체가 [surface-depth.md](references/surface-depth.md)의
> "이식 체크리스트"와 같다.
>
