...
> 선례: `difference.html`이 이 방식으로 전환됐다(나머지 페이지는 순차 적용 예정).
...
1. **배경은 페이지가 아니라 섹션이 갖는다** — 흰 면과 틴트 면을 번갈아 둔다.
   ...
   6% 차이뿐이라 경계가 안 보이고 `brand-tint-1`과 값이 같다(capability에서 같은
...
- **섹션마다 콘텐츠 폭을 바꾼다.** 우리는 9개 섹션이 전부 1280px 한 폭이라 리듬이
