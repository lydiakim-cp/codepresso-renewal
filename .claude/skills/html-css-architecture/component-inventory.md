...
| Pain Card | `pain-card.css` | `.pain-grid`(열 수는 `--pain-columns`, 기본 3), `.pain-card` | "이런 고민이 있다면"을 한 줄씩 담는 문제 제기 카드. 뒤의 해결책이 답으로 읽히게 하는 자리다. **자랑하는 수치가 아니므로 `metric-card`를 쓰지 않는다.** skillcertify·skillfit 공용 |
