...
> 읽는 법 — "언제 값을 토큰으로 승격하는가"(토큰 승격 기준 절)는 범용 판단
> 기준이다. 아래 "🔧 코드프레소 값 — 교체 지점" 블록의 실제 hex·px 값은
> 자사 값으로 바꾼다. → [porting-guide.md](porting-guide.md) 1단계
...
> 🔧 코드프레소 값 — 교체 지점 (아래 색상·spacing·radius·타이포·모션 값 전체)
...
> 이 절은 범용 방법론이다 — 어느 디자인 시스템에도 그대로 적용한다.
