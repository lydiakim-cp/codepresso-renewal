...
- 실패 사례: "조건 3개" 섹션을 정적 카드로 만들었는데, 메인의 `how-it-works`가 같은 역할을 `feature-card`로 하고 있었다. 교체 후 페이지 CSS가 40여 줄 사라졌다(`feature-card`는 이후 코드프레소 방침으로 전환 애니메이션을 전면 제거해 지금은 정적 카드다 — 그때 얻었던 것은 순환·hover 정지가 아니라 마크업·레이아웃의 재사용이었다는 뜻).
- **기존 컴포넌트를 다른 페이지에서 쓰려는데 특정 페이지 셀렉터에 묶여 있으면**(`.how-it-works .feature-card {...}`) 그 규칙을 컴포넌트 레벨로 올린다(`.feature-card`). 페이지 전용 CSS에 같은 규칙을 복사하지 않는다 — 올린 이유는 주석으로 남긴다.
