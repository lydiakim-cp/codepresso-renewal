 ...
 * - part-nav--animated로 감싸 인디케이터(data-catalog-indicator)가 있으면 활성 버튼 위치로 슬라이딩시킨다.
...
(() => {
  ...
  boards.forEach((board) => {
    ...
    const indicator = board.querySelector('[data-catalog-indicator]');

    const moveIndicator = (button) => {
      if (!indicator) return;
      indicator.style.width = `${button.offsetWidth}px`;
      indicator.style.transform = `translateX(${button.offsetLeft - buttons[0].offsetLeft}px)`;
    };

    const activate = (key) => {
      buttons.forEach((button) => {
        const isActive = button.dataset.catalogCategory === key;
        button.classList.toggle('is-active', isActive);
        if (isActive) moveIndicator(button);
      });
    ...
    buttons.forEach((button) => {
      button.addEventListener('click', (event) => {
        // <a href="#panel-id">로 놓인 경우 앵커 점프 대신 제자리에서 판만 바꾼다.
